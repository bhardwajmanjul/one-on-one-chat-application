...
const ChatPage = ()=> {
  ...
  stompClient.send(`/app/sendMessage/${roomId}`,{},JSON.stringify(message));
    ...
    return(
        ...
        <header className="dark:border-gray-700  fixed w-full dark:bg-blue-900 py-5 shadow flex justify-around items-center">
          {/* room name container */}
          <div>
 ...
 <div  className= { `my-2 ${
                message.sender === currentUser ? "bg-green-500" : "bg-gray-800"
              } p-2 max-w-xs rounded`}>

<div className="flex flex-row gap-2">
    <img className="h-10 w-10"src = {'https://avatar.iran.liara.run/public/27'}></img>
<div className="flex flex-col gap-1">
<p className="text-sm font-bold">{message.sender}</p>
<p>{message.content}</p>
<p className="text-xs text-white-200">{timeAgo(message.timeStamp)}</p>
      ...
      <div className=" fixed bottom-4 w-full h-16 ">
        ...
        <button className="dark:bg-purple-600 h-10 w-10  flex   justify-center items-center rounded-full"><MdAttachFile/> </button>
